...
const jobSchema = z.object({
  company: z
    .string()
    .min(1, "Institution selection is required")
  ...
  subject: z
    .string()
    .min(1, "Subject field is required")
    .transform((val) => val.trim()),
  qualificationRequired: z
    .string()
    .min(1, "Qualification field is required")
    .transform((val) => val.trim()),
  description: z
    .string()
  ...
  skillsRequired: z
    .string()
    .optional()
    .transform((val) => (val ? val.trim().replace(/,\s+/g, ",").replace(/\s+,/g, ",") : "")),
...
const PostAJobButton = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries(["school-jobs"]);
